...
public class MongoDBPlayer implements Serializable {
    ...
    /**
     * 找到若干个符合条件的数据，进行更新，如果没找到，则插入待更新的数据。
     *
     * @param filter
     * @param update
     */
    public void findAllAndAlwaysUpdate(Duality filter, Duality update) {
        UpdateOptions options = new UpdateOptions();
        options.upsert(true);
        currentCollection.updateMany(filter, update, options);
    }
    ...
    /**
     * 找到若干个符合条件的数据，进行更新，如果没找到，则什么都不做。
     *
     * @param filter
     * @param update
     */
    public void findAllAndUpdate(Duality filter, Duality update) {
